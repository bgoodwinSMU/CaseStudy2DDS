...
# Plot individuals
plot(quitters.pca.scaledPlots$x[,1], quitters.pca.scaledPlots$x[,2], xlab=paste("PCA 1 (", round(s$importance[2]*100, 1), "%)", sep = ""), ylab=paste("PCA 2 (", round(s$importance[5]*100, 1), "%)", sep = ""), pch=pch.group, col="black", bg=col.group, cex=1, las=1,xlim = c(-5,8),ylim = c(-5,8))
# Add grid lines
...
l.x <- cbind(quitters.pca.scaledPlots$rotation[,1][c(21, 24, 10, 14,25)]) * 10
# y coordinates (PC2)
l.y <- cbind(quitters.pca.scaledPlots$rotation[,2][c(15, 1, 27, 25)]) * 10

# Add arrows to biplot
arrows(x0=0, x1=l.x, y0=0, y1=l.y, col="red", length=0.15, lwd=3)
# Labels
text(l.x, l.y, labels=rownames(l.x) , col="red", pos=c(2, 1, 3, 1), offset=1, cex=1.2)
