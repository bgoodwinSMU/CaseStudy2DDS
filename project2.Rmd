...
#barplot(abs(quitters.pca.scaled$rotation[,1]),main="PC 1 Loadings Plot",las=2,ylim = c(0.0,0.6))+abline(0.30,0)
...
```{r}
####Testing biplot code####
#Create multiple graphs of loadings and attrition
#Plot ellipses of quit or no
#only plot loadings of most powerful 
...
# l.x <- quitters.pca.scaledPlots$rotation[,1]*10
# l.y <- quitters.pca.scaledPlots$rotation[,2]*10
# 
# 
# # Draw arrows
# arrows(x0=0, x1=l.x, y0=0, y1=l.y, col="red", length=0.15, lwd=1.5)
# # Variable labels
# text(l.x, l.y, labels=row.names(quitters.pca.scaledPlots$rotation), col="red", pos=l.pos)


# Create matrix of x coordinates (PC1) and multiply by 10
l.x <- cbind(quitters.pca.scaledPlots$rotation[c(21,24,10,14,15,1,27,25),1]) * 10
# y coordinates (PC2)
l.y <- cbind(quitters.pca.scaledPlots$rotation[c(21,24,10,14,15,1,27,25),2])*10

# Add arrows to biplot
arrows(x0=0, x1=l.x, y0=0, y1=l.y, col="red", length=0.15, lwd=3)
# Labels
text(l.x, l.y, labels=rownames(l.x) , col="black", pos=c(1,1,3,4,1,3,1,2), offset=1.5, cex=1.2)

# Screeplot
# Screeplot
layout(matrix(1:2, ncol=2))
screeplot(quitters.pca.scaledPlots)
screeplot(quitters.pca.scaledPlots, type="lines")

biplot(quitters.pca.scaledPlots,group="Attrition")
```
...
# Create matrix of x coordinates (PC1) and multiply by 10
l.x <- cbind(quitters.pca.scaledPlots$rotation[c(21,19,18,20,3,11,13,15,5),2]) * 10
# y coordinates (PC2)
l.y <- cbind(quitters.pca.scaledPlots$rotation[c(21,19,18,20,3,11,13,15,5),3])*10

# Add arrows to biplot
arrows(x0=0, x1=l.x, y0=0, y1=l.y, col="red", length=0.15, lwd=3)
# Labels
text(l.x, l.y, labels=rownames(l.x) , col="black", pos=c(3,1,3,1,1,2,1,1), offset=1.5, cex=1.2)
```
